...
describe("OpenCrowd session defaults", () => {
  it("upgrades the shipped CrowdCode default while preserving custom pins", async () => {
    const dir = join(await tempRoot(), "config");
    process.env.OPENCROWD_CONFIG_DIR = dir;
    await mkdir(dir, { recursive: true });
    for (const version of ["0.5", "0.4.0"]) {
      ...
      expect(config.mcpServers.crowdcode?.args).toEqual(["--yes", `crowdcode-mcp@${version === "0.5" ? "0.5.2" : version}`]);
